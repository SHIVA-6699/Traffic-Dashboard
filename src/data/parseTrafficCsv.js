...
/**
 * Fetch all 5 September CSVs and return one combined dataset + every aggregate needed for charts.
 */
export async function fetchFullWeekData(speedLimitKmh = SPEED_LIMIT_KMH) {
  const allRows = [];
  const dayNames = ['Sept 5', 'Sept 6', 'Sept 7', 'Sept 8', 'Sept 9'];
  ...
  const byDay = Array.from({ length: 5 }, () => ({ count: 0, overLimit: 0 }));
  const byEntry = {};
  dirOrder.forEach((d) => {
